...
void AddAlignmentComponent(Entity e){
    ...
    logTrace(printf("Alignment component allocated with address %p", e->components.CompAlignmentIndex), 1);
}
...
void CalculateAlignmentPosition(Entity alignment){
    for(int i = 0; i < alignment->components.alignment->spriteLen; i++){
        alignment->components.alignment->sprites[i]->position.x = (i * (alignment->components.alignment->cellSize.x + alignment->components.alignment->padding.x));
        alignment->components.alignment->sprites[i]->position.y = (i * (alignment->components.alignment->cellSize.y + alignment->components.alignment->padding.y));
    }
}
